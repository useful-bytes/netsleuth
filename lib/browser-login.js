...
exports.login = function(opts, cb) {
	...
	app.get('/logged-in', function(req, res) {
		...
		request({
			...
			if (opts.finished) res.redirect(opts.finished);
			else res.sendFile(path.join(__dirname + '/../www/success.html'));

			setTimeout(function() {
				var daemon = new Daemon(config);
				daemon.reload(function(err) {
					if (opts.openBrowser !== false) cb(err, username);
				});
			}, 1000);
	...
	app.get('/failed', function(req, res) {
		if (opts.openBrowser === false) res.redirect(opts.finished);
		else cb(new Error('Login failed or was cancelled.'));
	});
	...
	var server = app.listen(function(err) {
		...
		} else opn('http://127.0.0.1:' + port + '/' + (opts.welcome ? 'welcome' : 'login')).catch(cb);
	});
};