...
HAREntry.prototype.observeResBody = function(stream) {
	...
	stream.on('end', function() {
		if (self.resText.length) {
			self.resComplete(self.resText);
		} else {
			self.resComplete(Buffer.concat(self.resBody));
		}
	});
};

HAREntry.prototype.resComplete = function(body) {
	var self = this;
	...
	if (typeof body == 'string') {
		...
		self.out.response.content.text = body;
	} else {
		self.out.response.bodySize = body.length;
		self.out.response.content.size = body.length;
