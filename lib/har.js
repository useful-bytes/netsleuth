...
HAREntry.prototype.observe = function(req) {
	...
	for (var k in req._headers) headers.push({
		name: req._headerNames[k],
		value: req._headers[k].toString()
	});
...
HAREntry.prototype.observeReqBody = function(stream) {
	...
	stream.write = function(chunk, enc, cb) {
		if (typeof enc == 'string' || typeof chunk == 'string') self.reqBody.push(new Buffer(chunk, enc));
	...
	stream.end = function(chunk, enc, cb) {
		if (typeof chunk == 'string') self.reqBody.push(new Buffer(chunk, enc));
