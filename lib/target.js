...
Target.prototype.handleBin = function(type, id, payload) {
	...
	if (txn) {
		if (type == 1) {
			if (txn.reqBody && txn.reqBody.input.writable && !txn.reqBody.input.write(payload)) self.send({
				m: 'rp',
				id: txn.id
			});
			self.emit('req-data', txn, payload);
		} else if (type == 2) {
			if (txn.resBody && txn.resBody.input.writable && !txn.resBody.input.write(payload)) self.send({
				m: 'pp',
				id: txn.id
			});
		...
		case 'ws':
			...
			ws.on('open', function() {
				...
				self.cwss.handleUpgrade({
					...
					if (txn.rwsQueue) {
						txn.rwsQueue.forEach(function(item) {
							rws[item.op](item.data);
						});
						txn.rwsQueue = null;
					}
				});
			...
			// the origin can send messages before our client is ready, queue them
			function enqueue(item) {
				if (txn.rwsQueue) txn.rwsQueue.push(item);
				else txn.rwsQueue = [item];
			}

			ws.on('message', function(data) {
				if (txn.rws) txn.rws.send(data); // TODO: better flow control
				else enqueue({ op: 'send', data: data });
				self.emit('ws-frame-received', txn, data);
			});

			ws.on('ping', function(data) {
				if (txn.rws) txn.rws.ping(data);
				else enqueue({ op: 'ping', data: data });
			});

			ws.on('pong', function(data) {
				if (txn.rws) txn.rws.pong(data);
				else enqueue({ op: 'pong', data: data });
			});
