...
function Target(inspector, opts) {
	...
	secureAgentOpts.checkServerIdentity = function(hostname, cert) {
		var r = tls.checkServerIdentity(hostname, cert);	
		console.log('check Id', hostname, cert, r);
		return r
	};
	...
	switch (msg.m) {
		...
		case 'p':
			...
			if (txn) {
				...
				txn.resBody = new MessageBody(msg.id, msg.headers, {
		...
		case 'r':
			...
			if (self.forwardProxy) {
				...
				txn.targetHost = self.url.hostname;
			...
			req.on('socket', function() {
				if (self.forwardProxy) req.socket.on('connect', function() {
				...
				req.socket.on('secureConnect', function() {
			...
			req.on('response', function(res) {
				...
				txn.resBody = new MessageBody(msg.id, res.headers, {
					maxSize: self.opts.resMaxSize,
					kind: 'res',
					host: txn.originalHost
				});
