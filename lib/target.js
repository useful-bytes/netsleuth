...
Target.prototype.handleMsg = function(data, isParsed) {
	...
	switch (msg.m) {
		...
		case 'r':
			...
			if (txn.reqBody) {
				txn.reqBody.fileable = false;
				txn.reqBody.on('big', function() {
					if (!txn.req) {
						// We get here if we have buffered a large amount of client request body
						// and user interception script has not yet completed.
						self.send({
							m: 'rp',
							id: txn.id
						});
			...
			self.inspector.script.request(txn, function(err, mods) {
				...
				if (txn.reqBody) {
					if (txn.reqBody.length) req.write(txn.reqBody.data);
					txn.reqBody.fileable = true;
					if (txn.reqPaused) {
						self.send({
							m: 'rr',
							id: txn.id
		...
		case 'pr':
			var txn = self.inspector.reqs[msg.id];
			if (txn && txn.res) {
				if (typeof txn.res._pause == 'function') txn.res._pause();
				txn.res._pause = false;
			}
			break;
		...
		case 'wsping':
			var txn = self.inspector.reqs[msg.id];
			if (txn && txn.ws) txn.ws.ping(msg.d);
			break;

		case 'wspong':
			var txn = self.inspector.reqs[msg.id];
			if (txn && txn.ws) txn.ws.pong(msg.d);
			break;

		case 'wsclose':
			var txn = self.inspector.reqs[msg.id];
			if (txn && txn.ws) txn.ws.close();
			break;
