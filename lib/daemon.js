...
Daemon.prototype.spawn = function(cb) {
	...
	var args = [path.join(__dirname, '..', 'bin', 'daemon.js'), self._host.port];

	if (process.platform == 'win32') go('node');
	else if (self.opts.noAuthbind || (process.getuid && process.getuid() == 0)) go('node');
		...
		if (avail) {
			...
			if (process.platform == 'darwin' && !self.opts.noAuthbind) {
				console.warn('netsleuth: authbind not found.  Installing...');
				require('sudo-prompt').exec('node ' + path.join(__dirname, '..', 'bin', 'authbind.js'), { name: 'netsleuth authbind installer' }, function(err, stdout, stderr) {
					commandExists('authbind', function(err, avail) {
						if (err || !avail) {
							console.warn('netsleuth: authbind failed to install.  The netsleuth daemon will be unable to bind to privileged ports.');
							go('node');
						} else {
							console.warn('netsleuth: authbind installed successfully.');
							args.unshift('node');
							go('authbind');
						}
					});
