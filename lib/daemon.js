...
Daemon.prototype.spawn = function(cb) {
	...
	var localInstall = path.join(__dirname, '..', 'bin', 'daemon.js');

	// always prefer the globally installed version of netsleuth
	child_process.exec('npm config get prefix', function(err, prefix) {
		if (err || !prefix) cb(new Error('Failed to get npm global prefix: ' + (err && err.message)));
		else {
			prefix = prefix.trim();
			if (process.platform != 'win32') prefix = path.join(prefix, 'lib');
			var globalInstall = path.join(prefix, 'node_modules', 'netsleuth', 'bin', 'daemon.js');
			fs.access(globalInstall, fs.constants.R_OK, function(err) {
				if (err) exe(localInstall);
				else exe(globalInstall);
			});
		}

	});

	function exe(daemonPath) {
		var args = [daemonPath, self._host.port];

		if (process.platform == 'win32') go('node', args);
		else if (self.opts.noAuthbind || (process.getuid && process.getuid() == 0)) go('node', args);
		else commandExists('authbind', function(err, avail) {
	...
	function go(bin, args) {
		var proc = child_process.spawn(bin, args, {
			stdio: 'ignore',
			detached: true
		});
...
Daemon.prototype.add = function(opts, cb) {
	...
	request({
		...
		if (err) return cb(err);
		if (res.statusCode != 200) return cb(new Error((body && body.message) || 'HTTP ' + res.statusCode));
...
Daemon.prototype.findBestRegion = function(opts, cb) {
	var self = this;
	self.start(function(err) {
		if (err) cb(err);
		else request({
			method: 'POST',
			url: 'http://' + self.host + '/ipc/find-best-region',
			headers: {
				Origin: 'netsleuth:api'
			},
			agent: agent,
