...
function HTTPTransaction(target, msg, opts) {
	...
	if (target.forwardProxy) {
		var origin = url.parse(msg.url),
			proto = origin.protocol;

		this.targetProto = this.originalProto = proto.substr(0, proto.length-1);
		this.targetHost = this.originalHost = origin.host;
		this.targetPath = this.originalPath = origin.path;
